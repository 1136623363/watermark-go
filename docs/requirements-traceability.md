...
| R2 | 交付 Go 模块化单体，由 Go 承担 HTTP、业务、数据和调度，主业务不依赖旧后端。 | 计划任务 15 验证；实现分布在任务 2、3、5、7、8、9、11。API/helper/proxy/data-gate 均为同一模块化单体镜像的受限 role。 | `cmd/watermark-go/main.go`、`cmd/parser-helper/main.go`、`cmd/netguard-proxy/main.go`、`internal/app/app.go`、`internal/httpapi/router.go`、`internal/parser/descriptor.go`、`internal/parse/service.go`、`internal/task/worker.go`、`internal/store/gate_receipt.go` | `internal/app/app_test.go`、`internal/httpapi/router_test.go`、`internal/parser/registry_test.go`、`internal/parse/service_test.go`、`internal/task/worker_test.go`、Compose/network/receipt policy tests | `Dockerfile`、`artifacts/verification/local-verification.md`；runtime inspect 证明每个 role-set 的四个应用 role 使用同一 RepoDigest |
| R3 | 保持当前小程序的客户端 session、同步解析、异步解析、缓存分享、下载兜底、m3u8 和 performance 遥测契约，并以加法字段支持音频/图集/Live Photo。 | 计划任务 12、18 验证；接口实现分布在任务 3、6—9、11。 | `internal/auth/client.go`、`internal/parser/parser.go`、`internal/parse/normalize.go`、`internal/httpapi/client_handlers.go`、`internal/download/service.go`、`internal/media/m3u8.go`、`internal/observability/client_performance.go`、`docs/frontend-provenance.json` | `tests/contracts/frontend_contract_test.go`（含 `TestMediaParserIntegrationContract`）、`internal/parse/url_test.go`、`tests/e2e/test_frontend_flow.py`、`scripts/verify-frontend-provenance.sh` | `artifacts/acceptance/frontend-domain-e2e.json`、`artifacts/acceptance/media-parser-integration.json` |
...
| R5 | MySQL 持久化业务事实并保留旧数据；Redis 只用于缓存、锁和限流，故障时核心同步解析可降级。 | 计划任务 15 本地验证、任务 17 数据迁移/运行验证、任务 18 final 验收；存储实现由任务 5、7、8 完成。源 MariaDB 仅经精确 Unix socket 迁移；每个数据 stage 必须取得本 role 新 gate receipt。 | `internal/store/mysql.go`、`internal/store/legacy_import.go`、`internal/store/legacy_delta.go`、`internal/store/gate_receipt.go`、`internal/cache/redis.go`、`internal/parse/service.go`、`migrations/`、migration wrapper | `internal/store/migrate_test.go`、`internal/store/legacy_import_test.go`、`internal/store/legacy_delta_test.go`、`internal/cache/cache_test.go`；apply/revalidate、只读 privilege、stale receipt、Redis namespace、retained predecessor 负测 | `artifacts/migration/legacy-data-rehearsal.json`（MariaDB 11.8.6 经 `/run/mysqld/mysqld.sock` → 隔离 clone → MySQL 8.4、`chosenMigrationMode=final_full_no_binlog`、final full/import/checksum/no-writer）、每次 `--force-recreate --no-deps data-gate-${role}` 的 role/stage/digest/run/DB/Redis/outbox receipt、accepted-write predecessor/successor reconciliation、`artifacts/acceptance/redis-degraded.json` |
| R6 | GitHub Actions 执行测试与安全检查，以 Docker Buildx 构建并发布 GHCR 镜像。 | 任务 16 验证 recovery candidate A；任务 17 在 A 完整验收后生成并验证 promotion B；工作流由任务 13 完成。A→B tracked diff 唯一是 marker，OCI revision 仅是 CI label。 | `.github/workflows/ci-image.yml`、`Dockerfile`、`deploy/image-lock.json`、`scripts/promote.sh`、`release/promotion-marker.txt`、`scripts/verify-gitleaks.sh` | `internal/policy/docker_ci_test.go`、`scripts/verify-image.sh`；marker/evidence ref/parent/payload hash/A digest/run/diff allowlist 负测 | immutable `evidence/recovery-<runId>` ref、`artifacts/release/full-history-secret-scan.txt`、`artifacts/release/recovery-image-digest.txt`、`artifacts/release/final-image-digest.txt`、verified canonical `artifacts/release/image-digest.txt`、`artifacts/release/promotion-equivalence.json`、`artifacts/release/sbom-recovery.spdx.json`、`artifacts/release/sbom-final.spdx.json`、`artifacts/release/sbom.spdx.json` |
| R7 | 目标机只拉取不可变 GHCR manifest digest，并以单机 Docker Compose 部署；目标机不构建或导入应用镜像。 | 任务 17 建立并行隔离的 recovery/candidate role-set、verified recovery A、等价 final candidate B 与 B→A drill；任务 18 以 A rollback/B final 双 receipt 切换并观察 B。 | `deploy/compose.yml`、`deploy/image-lock.json`、`scripts/deploy-local.sh`、`scripts/rollback-local.sh`、`/var/lib/watermark-go/runtime.env`（服务器仓库外 0600） | `tests/ops/test_scripts.py`、`scripts/verify-image.sh`；同组 digest、专属 UDS/sandbox/receipt/data identity、force-recreate gate 与跨组串线负测 | `artifacts/acceptance/final-shadow-e2e.json`、`artifacts/deploy/pull-and-up.txt`（A/B role event ledger，最终含 B final up、actual RepoDigest、`localBuild=false/localLoad=false`）、`artifacts/deploy/state-before.json`（Task 18 锁定 A rollback/B final receipt hash）、`artifacts/deploy/running-digest.txt`、`artifacts/deploy/public-cutover.json`、`artifacts/release/repository-and-image.txt`（A/B role）；全部事件证明无本地 build/load |
...
| --- | --- | --- | --- |
| `ucmao/media-parser` 固定 commit/tree 中的集中 parser factory 与 50-domain alias 目录 | 任务 3：`internal/parser/descriptor.go`、`internal/parser/registry.go` | registry ASCII key/alias/domain 唯一性、稳定 priority/顺序、exact/label-boundary host 路由、constructor 零 I/O；每次 Parse 只取得一次有界 Snapshot，多阶段平台受 `MaxRequests`/`MaxRedirects` 限制并拒绝重复资源 URL | `docs/research/media-parser-provenance.json` 固定来源/license；50 aliases 仅为候选，不自动成为 production 支持项；当前 catalog 的 26 key/41 domain/21 ID 能力与 fixed-commit 41/41 controlled-subdomain 语义须精确锁定，不能把研究目录自动并入生产或静默缩小兼容面 |
| 集中目录应同时覆盖固定出口的可审计所有权 | 任务 4：`internal/netguard/authority.go` 与 native descriptor endpoint policy | `TestPurposeScopedOutboundAuthority`、`TestEveryNativeFixedEndpointHasPolicyOwner`、`TestParserAPIAuthorityCannotBeUsedAsInputRoute`、`TestSensitiveHeadersNeverReachDynamicMediaCandidateHost`、`TestCrossPurposeRedirectFailsClosed` | `InputShare`、`MetadataAPI`、`SessionBootstrap/SessionConsumer`、动态 `MediaCandidate` 互不替代；API authority 不得成为输入路由，动态 CDN 永不携带 session/Auth/Cookie |
| 平台必需 query 参数的显式保留 | 任务 3 先建立 `internal/netguard` 安全 core 并注入 parser，任务 4 封住全树/subprocess 出口，任务 7 实现 `internal/parse/url.go` | 表格测试覆盖 FetchURL/SafeURL/CacheKey、allowlist、tracking 剥离、IDNA/端口/恶意 host 后缀、首次请求前 SSRF、redirect/DNS rebinding、跨域敏感 header 剥离与日志脱敏 | Task 3/4 间无临时裸 HTTP adapter；canonicalizer 不联网/不盲改 HTTPS；raw query/capability 不进入日志、cache key 或 evidence，TLS 不得关闭 |
| video/gallery/audio/live-photo 与多媒体候选 | 任务 3 内部 `ImageAsset`/`MediaCandidate`，任务 7 兼容投影，任务 12 契约 | 单次有界 Snapshot（单页平台物理 fetch 恰好一次）、能力/结果一致、稳定质量排序、有界 fallback、旧 `images` 字符串数组不变、可选 `imageAssets` 配对 | 不盲信数组下标；多阶段平台仍受 descriptor 请求/跳转预算约束；每个媒体 URL 都经 netguard/media 门禁，不改变当前前端既有字段 |
| 结构化 API、嵌入 JSON 页面漂移和短期会话失效线索 | 任务 3 脱敏合成 golden 与 exact-host `SessionMaterialProvider`；任务 5/7 versioned singleflight cache/typed error；任务 8/9 bulkhead/异步媒体任务 | 截断/字段换层/登录/风控/空核心/INIT_STATE/Apollo fixture；只有 typed `session_expired` 至多刷新一次，TTL/singleflight/capacity/exact-host；缓存版本自动 miss、取消/凭据缺失不负缓存，DASH/ffmpeg 取消无泄漏 | import/constructor 无副作用，上游 session 与微信 client session 隔离，无源码会话 fallback；实时页面不能替代离线 fixture |
| music/Sohu 的可选凭据边界 | 任务 2/3 typed opaque config；music 仅识别 configured 状态，Sohu 只注入 API-native 窄 `TokenProvider` | 对 UDS/job、helper/child env、argv、日志、错误、output 分层 sentinel；缺凭据返回 `credential_required` | production 不存在 music helper consumer，依赖 secret 的 universal/music descriptor disabled；music/Sohu secret 均不得进入 helper/proxy，Sohu token 只能由 API-native provider 消费 |
...
最终 `artifacts/acceptance/media-parser-integration.json` 由 Task 13/16 可信 CI 结果和 Task 17/18 runtime
identity 共同生成，必须绑定 B `sourceCommit`、实际 `imageDigest`、`ciRunId` 与 test manifest/hash；
registry/structured JSON/query/candidate/cache/rich-media/unsafe-pattern 任一子门缺失即 R3/R6/R7 未关闭。
...
- 任务 17 先以独立 DB/schema、独立 Redis namespace 完成 A shadow 隔离全验；强制
  ...
  `A observation>=1800s` 后才成为 recovery digest。源 dump 只经已验证的 `/run/mysqld/mysqld.sock`、
  `network_mode:none` 与只读 bind，不能挂载源 data directory；每次 stage 先 force-recreate 对应 gate。
  A 首上失败只能恢复原 502 路由并标记 FAILED；一旦接受写入，确切 final DB/outbox 成为 retained fact
  source，重试必须 reconcile/复用或无丢失前移，只有 DB/outbox/route 三方证明零写才可重建。
- recovery/candidate 是可并存的两套 role-set；各组 API/helper/proxy/data-gate 使用组内同一 digest和专属
  UDS、sandbox、receipt、data identity。A/B shadow 使用不同 Redis namespace；drill/final/rollback 对账同一
  locked final namespace。所有 shadow/final/drill/cutover/rollback gate 都以本 run/attempt force-recreate，
  apply/revalidate 权限不可混用。
- A 验证后才允许 A→B source diff allowlist：tracked diff 唯一只改 `release/promotion-marker.txt`，不允许
  Go/依赖/Dockerfile/执行/config/migration/schema 变化。A/B 的 rootfs/app binary/tool versions/schema
  只允许 OCI label 白名单差异，OCI revision 是 CI label 而非 source diff；marker 绑定 immutable recovery
  evidence ref/OID/payload hash、A digest/source 与 promotionRunId，CI fetch 后重验。`recoveryDigest !=
  finalDigest`；A 是 recovery digest，B 是 final digest。
...
- 任务 18 在 fence 前以只读 `GATE_MODE=revalidate` force-recreate A recovery 与 B candidate gate，生成
  同一 attempt 的 A rollback receipt 和 B final receipt；两者绑定各自 digest/config、同一 final DB/schema/
  Redis namespace、当前 accepted-write/outbox 坐标，hash 锁入 state。旧/apply receipt 不可复用；B→A
  quick-check 只能消费坐标不回退且 identity 未漂移的预生成 A receipt。
