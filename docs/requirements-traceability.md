...
| --- | --- | --- | --- |
| `ucmao/media-parser` 固定 commit/tree 中的集中 parser factory 与 50-domain alias 目录 | 任务 3：`internal/parser/descriptor.go`、`internal/parser/registry.go` | registry ASCII key/alias/domain 唯一性、稳定 priority/顺序、精确 host 路由、constructor 零 I/O；每次 Parse 只取得一次有界 Snapshot，多阶段平台受 `MaxRequests`/`MaxRedirects` 限制并拒绝重复资源 URL | `docs/research/media-parser-provenance.json` 固定来源/license；50 aliases 仅为候选，不自动成为 production 支持项；当前 catalog 的 26 key/41 domain/21 ID 能力须精确锁定，不能把研究目录自动并入生产 |
