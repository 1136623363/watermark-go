...
func (x xiGua) parseShareUrl(shareUrl string) (*VideoParseInfo, error) {
	client := x.newRestyClientNoRedirect()
...
func (x xiGua) parseVideoID(videoId string) (*VideoParseInfo, error) {
	...
	headers := xiGuaRequestHeaders(x.xiguaCookie)

	client := x.newRestyClient()
	res, err := client.R().
		SetHeaders(headers).
		Get(reqUrl)
	if err != nil {
...
func xiGuaRequestHeaders(configured string) map[string]string {
	...
	if cookie := strings.TrimSpace(configured); cookie != "" {
