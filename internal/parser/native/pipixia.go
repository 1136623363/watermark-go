...
func (p piPiXia) parseShareUrl(shareUrl string) (*VideoParseInfo, error) {
	client := p.newRestyClientNoRedirect()
	// disable redirects in the HTTP client, get params before redirects
	res, err := client.R().
		SetHeader(HttpHeaderUserAgent, DefaultUserAgent).
		Get(shareUrl)
	if err == nil {
		return p.parseDirectURL(shareUrl)
	}
...
func (p piPiXia) parseDirectURL(shareUrl string) (*VideoParseInfo, error) {
	urlRes, err := url.Parse(shareUrl)
	if err != nil {
		return nil, err
	}
	videoId := strings.Trim(urlRes.Path, "/")
	videoId = strings.TrimPrefix(videoId, "item/")
	if len(videoId) <= 0 {
		return nil, errors.New("parse video id from share url fail")
	}
	return p.parseVideoID(videoId)
