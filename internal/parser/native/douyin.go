...
type douYin struct{ legacyHTTPClients }
...
func (d douYin) parseVideoID(videoId string) (*VideoParseInfo, error) {
	...
	if !isNote {
		...
		previewUrl = d.buildPreviewURL(data.Get("video.play_addr.uri").String())
...
func (d douYin) parseAppShareUrl(shareUrl string) (*VideoParseInfo, error) {
	...
	client := d.newRestyClientNoRedirect()
	...
	// 西瓜视频解析方式不一样
	if controlledHostname(locationRes.Hostname(), "ixigua.com") {
		// The redirect is still one Douyin Parse operation. Preserve the
		// guarded clients, absolute deadline and request/redirect budget when
		// delegating to the Xigua decoder.
		return (xiGua{legacyHTTPClients: d.legacyHTTPClients}).parseVideoID(videoId)
	}
...
func (d douYin) buildPreviewURL(uri string) string {
	...
	query := url.Values{}
	query.Set("video_id", uri)
	query.Set("ratio", "720p")
	query.Set("line", "0")
	query.Set("is_play_url", "1")

	return "https://www.douyin.com/aweme/v1/play/?" + query.Encode()
}
