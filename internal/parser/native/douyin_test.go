...
func Test_douYin_buildPreviewURL(t *testing.T) {
	d := douYin{}

	got := d.buildPreviewURL("v0300f440000b123456789")
	want := "https://www.douyin.com/aweme/v1/play/?is_play_url=1&line=0&ratio=720p&video_id=v0300f440000b123456789"
	if got != want {
		t.Fatalf("buildPreviewURL() = %q, want %q", got, want)
	}

	if d.buildPreviewURL("") != "" {
		t.Fatalf("buildPreviewURL() should return empty string for empty uri")
	}
}
