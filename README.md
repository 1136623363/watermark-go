...
当前仓库仍处于实现计划执行阶段：任务 1 的仓库安全边界已完成，任务 2 及后续功能不得在对应测试和验收证据完成前标记为交付。
...
模块语言版本保持为 `go 1.24.0`，首选构建工具链固定为 `go1.26.5`。任务 2 会完成模块重命名、工具链获取策略与应用骨架；在此之前，当前兼容树仍使用旧入口，不能当作最终结构。
